    ...
    for line_num, l in enumerate([line.strip().split(' ') for line in open(file_path, "r").readlines()]):
        ...
        def find_coeff_limits():
            ...
            while changed:
                ...
                def set_max_limits(eq):
                    changed = False
                    index = 1 # max limit
                    for i in range(len(limits)):
                        if is_zero(eq[0][i]):
                            continue
                        lim_num = min(limits[i][index], eq[1] // eq[0][i])
                        if lim_num >= 0 and limits[i][index] != lim_num:
                            limits[i][index] = lim_num
                            changed = True
                    return changed

                def set_min_limits(eq):
                    changed = False
                    for i in range(len(limits)):
                        if is_zero(eq[0][i]):
                            continue
                        ...
                        for j in range(len(limits)):
                            if i == j:
                                continue
                            r_value -= eq[0][j] * limits[j][1]

                        value = r_value // eq[0][i]
                    ...
                    if has_pos_coeffs and has_neg_coeffs:
                        ...
                        for i in range(len(limits)):
                            if eq[0][i] > 0:
                                ...
                                tmp_min_eq0[1] -= eq[0][i] * limits[i][0]
                                tmp_max_eq0[0][i] = 0
                                tmp_min_eq0[0][i] = 0
                            elif eq[0][i] < 0:
                                tmp_max_eq1[1] -= eq[0][i] * limits[i][1]
                                tmp_min_eq1[1] -= eq[0][i] * limits[i][0]
                                tmp_max_eq1[0][i] = 0
                                tmp_min_eq1[0][i] = 0

                        if set_max_limits(tmp_max_eq0): changed = True
        ...
        def rec():
            ...
            if len(coeffs) != len(eqs[0][0]):
                ...
                for eq in eqs:
                    for eq_index, eq_value in enumerate(eq[0]):
                ...
                for limit_value in smallest_limits:
                    ...
                    for limit_range in range(limit_value[1][0], limit_value[1][1] + 1):
